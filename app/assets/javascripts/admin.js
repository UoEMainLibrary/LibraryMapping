...
$(window).load(function () {
    if ($('body').attr("data-controller") != 'admin') return;
    /* ------- CANVAS PROPERTIES ------- */
    ...
    saveElementData = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.notify("Shelf attributes saved successfully", "success");
            },
            error: function (data) {
                $.notify("Error saving element " + data.error + "", "warn");
            }
        });
    };
    ...
    selectShelf = function (id) {
        var shelf = canvas.getObjects().find(function (o) {
            return o.id == id
        });
    ...
    printThis = function () {
        ...
        if (count == 2) {
            ...
            var element1 = canvas.getObjects().find(function (o) {
                return o.id == $(".print_col input:checked:eq(0)").data("id")
            });
            var element2 = canvas.getObjects().find(function (o) {
                return o.id == $(".print_col input:checked:eq(1)").data("id")
            });
    ...
    canvas.on('object:selected', function (options) {
        // The case when you disselect a shelf and select a new one
        if (selected_item && selected_item.element_type_name == "Shelf") {
            selected_item.set('fill', 'rgb(37,22,157)');
        }
        selected_item = options.target;
        $("#remove-selected").css("display", "initial");
        $("#shelfData").css("display", "none");
        if (options.target.element_type_name == "Shelf") {
            selected_item.set('fill', 'red');
            ...
            // Faking reload effect in order to draw attention to the form
            $('#shelfData').fadeTo('fast', 0.2).fadeTo('fast', 1.0);
        }
    });

    // Toggle frontend form, when selection is cleared
    canvas.on('selection:cleared', function () {
        if (selected_item.element_type_name == "Shelf") {
            selected_item.set('fill', 'rgb(37,22,157)');
        }
